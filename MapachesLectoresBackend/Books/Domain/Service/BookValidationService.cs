...
)
{
    ...
    public async Task<DataResult<(Book?, Publisher, IEnumerable<Author>, IEnumerable<Category>)>> ValidateBookRequestAsync(CreateBookRequestDto request, bool bookWithExisitingNameShouldNotExists = true)
    {
        var spec = new BookSpecifications.GetByName(request.Name);

        var bookWithName = await bookRepository.GetFirstAsync(spec);
        
        if(bookWithExisitingNameShouldNotExists && bookWithName != null)
            return DataResult<(Book?, Publisher, IEnumerable<Author>, IEnumerable<Category>)>.CreateFailure(new ResourceAlreadyExists(nameof(Book)));

        var publisherResult = await getPublisherByUuidUseCase.InvokeAsync(request.PublisherId);
        if (publisherResult.IsFailure)
            return DataResult<(Book?, Publisher, IEnumerable<Author>, IEnumerable<Category>)>.CreateFailure(CreateBookErrors.PublisherNotFound_400());

        var publisher = publisherResult.SuccessResult.Data;

        if (!request.Authors.Any())
            return DataResult<(Book?, Publisher, IEnumerable<Author>, IEnumerable<Category>)>.CreateFailure(CreateBookErrors.AuthorNotFound_400());

        if (request.Authors.Count > MaxAuthors)
            return DataResult<(Book?, Publisher, IEnumerable<Author>, IEnumerable<Category>)>.CreateFailure(CreateBookErrors.ToManyAuthors_400());

        if (!request.Categories.Any())
            return DataResult<(Book?, Publisher, IEnumerable<Author>, IEnumerable<Category>)>.CreateFailure(CreateBookErrors.CategoriesNotFound_400());

        if (request.Categories.Count > MaxCategories)
            return DataResult<(Book?, Publisher, IEnumerable<Author>, IEnumerable<Category>)>.CreateFailure(CreateBookErrors.ToManyCategories_400());
        ...
        return DataResult<(Book?, Publisher, IEnumerable<Author>, IEnumerable<Category>)>.CreateSuccess((bookWithName, publisher, authors, categories));
    }
